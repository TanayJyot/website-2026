...
- We use [[MITx course - Simple Programs - Functions]] to reduce the amount times we need to write code.

- [[MITx course - Abstraction]]
- [[Gall's Law]]
- [[MITx course - Knowledge Base - Decomposition]]

- When [[MITx course - Knowledge Base - Decomposition]] and [[MITx course - Abstraction]] are used together code can be used many times when it has been debugged only once

# Introducing Functions:
[[MITx course - Simple Programs - Functions]] do not ‘function’ unless they are called upon.
- It has:
	1. A name 
	2. [[MITx course - Knowledge Base - parameter]]
	3. [[MITx course - Knowledge Base - docstring]]
	4. body

![[MITx course - Pictures - Pasted image 20230408203206.png]]
...
z= f(x)
```
In this case the actual parameter value i.e. x =3 is bound to the formal parameter x. Any operations done on x in the actual parameter will be reflective to x in the formal parameter.[[MITx course - Knowledge Base - parameter]]

We create a **scope** within a function when we enter a function. Wherein, instead of having [[MITx course - Knowledge Base - global scope]] we have a [[MITx course - Knowledge Base - variable scope]].
...
## return vs. print:
![[MITx course - Pictures - Pasted image 20230408212023.png]]
...
![[MITx course - Pictures - Pasted image 20230410113240.png]]
...
# Keyword Arguments:
- Generally the order in which the [[MITx course - Knowledge Base - parameter]] of a function are written matters. However you can bind values directly if you forget the order. 
...
- [[MITx course - Knowledge Base - Recursion]]
- Looping algorithms have variable that update through the loop.[[MITx course - Python Basics - Core Elements Of Programs#Iteration:]] 
...
- When the frame goes back to the previous frame/[[MITx course - Simple Programs - Functions#Calling Functions and Scope:]] to get the return value is when the multiplication occurs with the value of x in that frame. \
- The value is being returned to the previous function that asked for it and that is when the multiplication occurs. 
![[MITx course - Pictures - Pasted image 20230410133411.png]]
...
## Iteration vs Recursion:
![[MITx course - Pictures - Pasted image 20230410133838.png]]

# Inductive Reasoning:

- To stop a recursive function it eventually must reach the value designated and it will do so because the [[MITx course - Knowledge Base - parameter]] of the function are decreasing. 

- We can use [[MITx course - Knowledge Base - Mathematical Induction]] to check whether a function will ever end or not. 

![[MITx course - Pictures - Pasted image 20230504173827.png]]
- 2023-08-18: The way I would now think about this is that the [[PUMP 2 - Knowledge Base - Natural Numbers]] do have a starting point which is finite and that is the basis of [[MITx course - Knowledge Base - Mathematical Induction]] working here. 
# Towers of Hanoi 
...
![[MITx course - Pictures - Pasted image 20230505230733.png]]
...
- You can give [[MITx course - Knowledge Base - parameter]] default values and bind them to a default value. 
- We use [[MITx course - Knowledge Base - docstring]] as a way to tell the user what we are trying to do and what is supposed to happen. 
- [[MITx course - Knowledge Base - Recursion]] basically creates multiple [[scopes]] with various values and then uses these scopes (with different values) to solve the problem.
- [[MITx course - Knowledge Base - Recursion]] is simpler from the user side but it is more resource consuming from the computer side because of creating so many [[scopes]].
- Mostly easier when there is a mathematical #formula. 

___
# Interface
- This would be the [[Function Header]] and [[MITx course - Knowledge Base - docstring]]. 
